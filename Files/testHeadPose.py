...
while True:
    ...
    for rect in rects:
        ...
        if ear < EYE_AR_THRESH:
            ...
            if COUNTER > 25:
                playsound('sound.mp3')
                cv2.putText(frame, "WAKE UP", (180, 180), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
        ...
        if mou > MOUTH_AR_THRESH:
            ...
            if COUNTER_M == 40:
                TOTAL_M += 1
                detected_mouth_is_opened = datetime.now()
                mouth_time.append(detected_mouth_is_opened)
                print(mouth_time)
            elif COUNTER_M > 40:
                # playsound('sound.mp3')
                cv2.putText(frame, "Mouth is opened", (180, 280), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)

        else:
            # if COUNTER_M <= MOUTH_AR_THRESH:
            counter_arr_mou.append(COUNTER_M)
            COUNTER_M = 0

       
        if len(mouth_time) > 2:
            for i in range(len(mouth_time)-2):
                if (mouth_time[i + 2] - mouth_time[i]).total_seconds() <= 60:
                    print(mouth_time)
                    playsound('mouth_sound.mp3')
                    mouth_time = []
                elif (mouth_time[i + 1] - mouth_time[i]).total_seconds() >= 60:
                    mouth_time = [mouth_time[i + 1], mouth_time[i + 2]]
                    break
                else:
                    mouth_time = [mouth_time[i + 2]]
                    break
                TOTAL_M += 1
        ...
        if p2[0] < 200 or p2[1] > 300:
            ...
            if ear < EYE_AR_THRESH:
                ...
                cv2.putText(frame, "ROAD", (180, 280), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
    ...
    # cv2.normalize(frame, frame, 0, 500, cv2.NORM_MINMAX)    # повышает яркость изображения
...
plt.plot(x, ear_arr, label='EAR')
plt.title('График зависимости EAR от времени')
plt.xlabel("Время, с", fontsize=16)
plt.ylabel('EAR', fontsize=16)
plt.grid(True)
horiz_line_data = np.array([EYE_AR_THRESH for i in range(len(x))])
plt.plot(x, horiz_line_data, 'r--', alpha=0.8, label='Референсное значение')
plt.legend()

plt.figure()
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
x1 = np.linspace(0, t, len(mar_arr))
plt.plot(x1, mar_arr, label='EAR')
plt.title('График зависимости MAR от времени')
plt.xlabel("Время, с", fontsize=16)
plt.ylabel('MAR', fontsize=16)
plt.grid(True)
horiz_line_data = np.array([MOUTH_AR_THRESH for i in range(len(x))])
plt.plot(x, horiz_line_data, 'r--', alpha=0.8, label='Референсное значение')
plt.legend()
